...
The program consists of 5 python scripts. To solve the Schroedinger Equation for a certain potential
...
When executing main.py you are given the choice to change the default settings. These include:
    - The directory of the schrodinger.inp file (default: ".")
...
However the program should do a good job even without changing the default settings (given that
schrodinger.inp is in the default directory).

Besides saving the results in .dat files, the program plots the calculated results. It will generate 2 diagrams.
The left one contains the interpolated potential, the eigenstates (shifted on the y-axis by the eigenvalues)
